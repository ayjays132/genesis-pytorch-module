...
The self-replay buffer, denoted as \$B\$, is implemented as a limited-size memory (e.g., a deque). It stores tuples of the form \$(h, y)\$, where \$h\$ is a salient internal representation (e.g., a final hidden-state vector encoding a recent context window) and \$y\$ is the associated target or outcome (e.g., the correct label or next token). Each training iteration, the model's forward pass produces a hidden representation \$h_t\$ for the current input and a predicted output \$\hat{y}_t\$. We identify "meaningful" contexts to store, such as those with high training loss (indicating novel or challenging cases) or rich semantic content. The hidden state \$h_t\$ is *detached* (to prevent backpropagation through time into the buffer) and saved along with its ground truth \$y_t\$ into \$B\$. If \$B\$ is full, older entries are dropped, typically in a FIFO (First-In, First-Out) manner or based on a priority (e.g., replacing less informative contexts).
